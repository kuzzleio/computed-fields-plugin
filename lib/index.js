function computeField(document, sourceFields, valueExpression) {
  ...
  sourceFields.forEach((e) => {
    ...
    value = value.replace(r, getDocumentValue(document, e))
  })
  return value
}

function getDocumentValue(document, fieldname) {
  ...
  for (let field of fields) {
    ...
    {
      throw new Error(`Document ${JSON.stringify(document)} doesn't contain field '${fieldname}'`)
    }
  }
  return value
}
...
class CorePlugin {
  ...
  constructor () {
    ...
    this.routes = [
      ...
      {verb: 'post', url: '/:_id/_delete', controller: 'computedFields', action: 'delete'}, // TODO: test using _id to avoid having to check where to find doc id when handling action
  ...
  createComputedField (request) {
    ...
    return Promise.reject('Body must contain the following fields: \'name\', \'index\', \'collection\' and \'value\'')
  }

  deleteComputedField (request) {
    let _id = request.input.args._id || request.input.resource._id
    if (_id) {
      return this.removeComputedField(_id)
        .then(r => {
          return this.loadComputedFields() 
        })
    }
    return Promise.reject('Request must specify the \'_id\' of the computed field to delete')
  }
