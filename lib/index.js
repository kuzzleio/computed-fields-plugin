...
class CorePlugin {
   ...
   *
   * Workflow is:
   *  - Kuzzle load plugins in <kuzzle install dir>/plugins/enabled/* and try to instanciate all ones found
   *    during this step, plugin is only loaded in kuzzle memory (constructor is called)
   *  - Kuzzle try to initialiate worker plugins (this.config.threads = Number > 0) in spawned process thought pm2
   *  - "init" function is called for both woker and regular plugins.
   *  - Plugin manager register all plugin features into kuzzle (register hooks/pipes, load custom configuration, check for authentication strategies, ...)
   *
   */
  constructor () {
    /**
    ...
    /**
     * Define if this plugin support worker plugins (multi-threaded)
     * Setting this to true allow users to set "threads" configuration
     *
     * /!\ warning: allowing threads result in incapacity to inject:
